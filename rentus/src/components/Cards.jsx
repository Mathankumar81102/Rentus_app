...
function Cards(props) {
  ...
  const fullDate =  `${year}-${month}-${day}`;
    if(value==true){
      Axios.post ("http://localhost:3001/setStatus",{byer:localStorage.getItem("user"),product:props.data.name,seller:props.data.user,rentDate:fullDate}).then((response)=>{
  ...
  return (
    ...
    <Link className='border-none  rounded-none px-0  py-0' state={{data:props.data,user:props.user}}  to={"/Home/product/"+props.id}><div className='  gap-x-44  rounded-2xl hover:scale-y-110 hover:scale-x-105 ease-in-out border-2 border-gray-500 flex  flex-col my-3   max-w-full min-w-0 h-full mt-3 col-span-1 pb-3 bg-zinc-50'>
    ...
    <button onClick={handleClick} className=' hover:animate-wiggle p-1 bg-emerald-50 mx-auto mt-8 hover:font-bold hover:border-black'>Rent Now</button>
